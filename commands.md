...
因為這個命令是藉由操作BIOS的keyboard buffer來達成相關的目的，所以有可能在某些系統上會造成當機，重啟或是其它不可預知的行為。如果是在GRUB之後的作業系統或是bootloader是用自己的keyboard driver的話，或者是使用BIOS的keyboard functions，在這些案例之下則這個命令將不會有任何的效果，而且還有一個限制就是這個命令只在PC BIOS下才有效果
