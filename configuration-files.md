...
### 10_linux
這個script會辨識目前在使用中的作業系統，root device中的kernel版本，並且建立相關的menu entry，如果有允許recovery mode的話也會將其寫入menu entry。在GRUB 1.99之後，規則就變成只有最新的kernel才會顯示在主要的menu上，其他的版本都會顯示在『submenu』裡面，不然當每次kernel更新時，你會發現你的menu越長越多是一件很恐怖的事。並且，submenu不會存在在non-Linux的作業系統上或是其它的partitions上的kernel。
...
sudo update-grub
```
<font color="red">不過建議使用『GRUB_DISABLE_OS_PROBER』就好，盡量不要去設定這個檔案的『executable bit』</fond>
