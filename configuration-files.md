...
這個檔案你實際去看會分很多個section，每個section都會使用『###BEGIN###』開始，和使用『###END結束』，代表的是每個『/etc/grub.d』底下的檔案。

在早期的GRUB2版本，這個檔案其實是唯讀的，即使連『root』也不例外，但是因為開發者發現有時候簡單的改變這個檔案會對使用者比較方便，所以後來就開放可以編輯，這部份使用者自己要了解的是，當你或系統執行『update-grub』時，之前的修改又會被覆寫掉就對了，所以，最好還是由上圖的流程去修改你要的檔案，在執行『update-grub』會比較正規。而且官方上面還有說如果你自己手動修改的話，有可能造成『update-grub』執行時會造成更新出問題，導致無法更新grub.cfg，所以再次強調，可以的話還是造正規途徑修改吧。
