...
## Script : /etc/grub.d/
在這個資料夾裡面的檔案都是script，執行的結果都會輸出到『grub.cfg』裡，像之前說過的每個檔案的內容都會以『###BEGIN###』開始，和使用『###END###』結束，因為這個資料夾裡面檔案的格式前面都會有個兩碼的數字，而執行的順序會以這個數字為主，數字越低的越先執行，像是『10_linux』就會比 『20_memtest』還要快執行，如果有開頭非數字的script的話，則這個檔案就會在所有數字開頭的script執行完以後才會執行。
