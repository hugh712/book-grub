...
使用這個custom menu有底下幾個優點:
1.只有在使用者修改他時才會更新，意思是當你的套件還是kernel更新時不會影響到它的位置。
2.menuentry的標題很好改動。
3.在menu裡面就不用管順序。
4. 可以特別為指定的entry設定密碼保護。

有優點當然就有缺點，缺點如下:
1.缺少更新機制。
2.有些『/etc/default/grub』裡面的使用者資訊可能會抓不到。
