...
像之前說過的『grub.cfg』裡面的內容會有很多個section，每個section都會以『###BEGIN###』開始，和使用『###END###』結束，這每個section的內容都是在這個資料夾裡面輸出的，執行順序的話，因為這個資料夾裡面檔案的格式前面都會有個兩碼的數字，而執行的順序會以這個數字為主，數字越低的越先執行，像是『10_linux』就會比 『20_memtest』還要快執行，如果有開頭非數字的script的話，則這個檔案就會在所有數字開頭的script執行完以後才會執行。
